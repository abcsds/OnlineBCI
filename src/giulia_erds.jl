...
fname = "raw/subj_vamo24_block1.xdf"
...
for sn in 1:n_streams
    ...
    elseif streams[sn]["name"] == "BrainVision RDA"
        ds = sn
    else
        continue
    end
...
title!("Spectrogram")
savefig("doc/img/GSingle_Spectrogram.png")
...
vline!([0], c="black", lab=nothing)
title!("ERDS map")
savefig("doc/img/GSingle_ERDS_map.png")
...
plot(map_times, S.freq, mean(maps, dims=1)[1,:,:], xlab="Time (s)", ylab="Frequency (Hz)", fill=true, c=:viridis)
vline!([0], c="black", lab=nothing)
title!("average ERDS map")
savefig("doc/img/GAvgERDS.png")

plot(map_times, S.freq, mean(maps, dims=1)[1,:,:], ylim=(0,20), xlab="Time (s)", ylab="Frequency (Hz)", fill=true, c=:viridis)
vline!([0], c="black", lab=nothing)
title!("average ERDS map")
savefig("doc/img/GAvgERDS_020.png")
...
# savefig("doc/img/GAvgERDS_Utest.png")
